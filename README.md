...
- **修复书架更新检测卡在旧章节数的问题**：此前检测更新直接采用服务器 `/api/updates` 的 `has_update` 标志，而该标志表示「服务器库 vs 番茄源」。当服务器已自行补全完整副本（如下载服务器清理缓存后重新下载），该标志恒为 `false`，导致本地副本落后的书籍永远无法更新。现改为用服务器返回的最新章节数（`remote_total`/`local_total`）与本地已下载章节数比较来判断是否下载。
